...
const Navbar = () => {
  ...
  const navItems = [
    ...
    { name: "Masjids & Madrasas", href: "#masjids-madrasas" },
          ...
          <div className="hidden lg:flex items-center space-x-8">
            {navItems.map((item) => (
              <button
                ...
                className={`text-slate-600 hover:text-blue-600 dark:text-slate-300 dark:hover:text-blue-400 transition-all duration-200 relative ${activeSection === item.href.substring(1)
            ...
            <div className="flex flex-col space-y-3">
              {navItems.map((item) => (
                <button
                  key={item.name}
                  onClick={() => handleNavClick(item.href)}
                  className={`text-slate-600 hover:text-blue-600 dark:text-slate-300 dark:hover:text-blue-400 transition-colors duration-200 py-2 text-left ${activeSection === item.href.substring(1)
                      ? 'text-blue-600 dark:text-blue-400 font-medium'
                      : ''
                    }`}
                >
